...
#define NUM_SAMPLE 16			// 訓練データのサンプル数。
#define NUM_INPUT 4				// 入力ノード数。
#define NUM_HIDDEN 3			// 中間層（隠れ層）の素子数。
#define NUM_OUTPUT 15			// 出力素子数。
...
int main(void)
{
	int    ilearn, isample, i;
	double error, max_error;
	FILE   *stream;
	unsigned long init[4]={0x123, 0x234, 0x345, 0x456}, length=4;
	unsigned long seed=1245848422;	// この乱数シードを毎回変えて実験したい場合は「コマンドライン引数」で受け取るよう変更すること。
	
	// 訓練データのファイルからの読み込み。
	stream = fopen("mac_training2.dat", "r");

	if(stream == NULL)
	{
		printf("mac_training2.dat cannot open!!\n");
		exit(0);
	}
 	else
	{
...
void ReadData(FILE *stream2)
{
	...
	for(isample=0; isample<NUM_SAMPLE; isample++)
	{
		...
		{
			fscanf(stream2, "%d", &tx[isample][i]);
		}

		for(i=0; i<NUM_OUTPUT; i++)
		{
			fscanf(stream2, "%d", &ty[isample][i]);
		}
	}
	...
	for(isample=0; isample<NUM_SAMPLE; isample++)
	{
		printf("training data: NO. %d : ", isample+1);
		printf("input data: ");
		for(i=0; i<NUM_INPUT; i++)
		{
